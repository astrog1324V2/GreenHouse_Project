...
ESP32_C3_GIFT_CONFIG_PATH = (
    Path(__file__).resolve().parents[1]
    / "esp32"
    / "boards"
    / "esp32_c3_gift_display"
    / "app_config.py"
)


class AppCoreConfigTestCase(unittest.TestCase):
    def _load_board_config(self, path: Path):
        module_name = "test_board_config_%s" % len(self._cleanups)
    ...
    def test_component_mode_uses_temp_server_url(self) -> None:
        module = self._load_module(
            {
                ...
                "UPLOAD_ENABLED": True,
                "TEMP_WINDOWS_SERVER_URL": "http://10.0.0.15:8000/api/v1/readings",
        ...
        self.assertEqual(
            module._server_url(),
            "http://10.0.0.15:8000/api/v1/readings",
        )
        ...
        module = self._load_module(
            {
                "RUN_MODE": "component_test",
                "UPLOAD_ENABLED": True,
                "TEMP_WINDOWS_SERVER_URL": "https://10.0.0.15:8000/api/v1/readings",
                "SERVER_URL": "http://192.168.1.50:8000/api/v1/readings",
                "WIFI_SSID": "lab-wifi",
                "WIFI_PASSWORD": "secret",
            }
    ...
    def test_esp32_c3_gift_display_profile_is_display_only(self) -> None:
        config = self._load_board_config(ESP32_C3_GIFT_CONFIG_PATH)

        self.assertEqual(config.RUN_MODE, "display_only")
        self.assertFalse(config.UPLOAD_ENABLED)
        self.assertEqual(config.DHT_PIN, 4)
        self.assertEqual(config.I2C_SDA_PIN, 8)
        self.assertEqual(config.I2C_SCL_PIN, 9)
        self.assertTrue(config.OLED_ENABLED)
        self.assertEqual(config.OLED_LAYOUT, "data_only")

        module = self._load_module(
            {
                "RUN_MODE": config.RUN_MODE,
                "UPLOAD_ENABLED": config.UPLOAD_ENABLED,
                "DISPLAY_REFRESH_SECONDS": config.DISPLAY_REFRESH_SECONDS,
        ...
        self.assertIsNone(module._upload_preflight_error())
        self.assertEqual(module._interval_seconds(), config.DISPLAY_REFRESH_SECONDS)
